...
for problem in "${ROOT}/test/test-suite/rte/"*; do
    problem_name=$(basename "${problem}")

    npx rekarel compile "${problem}/code.kcode" -o "${ROOT}/bin/${problem_name}.kx"
    ...
    for casename in "${problem}/cases"/*.in; do
        ...
        npx rekarel run "${ROOT}"/bin/${problem_name}.kx <"$casename" >"$actual_stdout" 2>"$actual_stderr"
        exit_code=$?
